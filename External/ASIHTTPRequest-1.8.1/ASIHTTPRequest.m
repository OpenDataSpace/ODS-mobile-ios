...
- (void)buildPostBody
{
	...
	if ([self postLength] > 0) {
		...
		if ([self.requestHeaders objectForKey:@"Transfer-Encoding"] == nil) {
...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (![self validatesSecureCertificate]) {
            ...
            // updated 13/02/2013 from https://github.com/ignaval/asi-http-request/commit/c782abbeb204156d30ecbb902915d1eaf9b10f9e
...
#if DEBUG_THROTTLING
                ASI_DEBUG_LOG(@"[THROTTLING] ===Reducing WWAN throttle to %ld bytes/second because of low bandwidth in %lu measurement cycles, bandwidth used in last second was %lu", ASIWWANLowBandwidthThrottleAmount, wwanBandwidthStrikes, bandwidthUsedInLastSecond);
#endif
