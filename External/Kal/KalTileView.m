...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextTranslateCTM(ctx, 0, self.tileSize.height);
    CGContextScaleCTM(ctx, 1, -1);
    
    if ([self isToday] && self.selected) {
        [[[UIImage imageNamed:@"kal_tile_today_selected.png"] stretchableImageWithLeftCapWidth:6 topCapHeight:0]
         drawInRect:CGRectMake(0, -1, self.tileSize.width+1, self.tileSize.height+1)];
        textColor = [UIColor whiteColor];
        shadowColor = [UIColor blackColor];
        markerImage = [UIImage imageNamed:@"kal_marker_today.png"];
    } else if ([self isToday] && !self.selected) {
        [[[UIImage imageNamed:@"kal_tile_today.png"] stretchableImageWithLeftCapWidth:6 topCapHeight:0]
         drawInRect:CGRectMake(0, -1, self.tileSize.width+1, self.tileSize.height+1)];
        textColor = [UIColor whiteColor];
        shadowColor = [UIColor blackColor];
        markerImage = [UIImage imageNamed:@"kal_marker_today.png"];
    ...
    textY = 6.f + roundf(0.5f * (self.tileSize.height - textSize.height));
    if (shadowColor) {
        [shadowColor setFill];
    ...
    if (self.highlighted) {
        ...
        CGContextFillRect(ctx, CGRectMake(0.f, 0.f, self.tileSize.width, self.tileSize.height));
    }
}
