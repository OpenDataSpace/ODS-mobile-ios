...
- (id)initWithAsset:(ALAsset *)theAsset andDelegate:(id<AGIPCGridItemDelegate>)theDelegate
{
    ...
    if (self)
    {
        [self performInitOnMainThread:^{
            ...
            // Must come after the views have been created
            [self setAsset:theAsset];
        }];
    }
...
- (void)performInitOnMainThread:(void (^)())block
{
	[self performSelectorOnMainThread:@selector(callBlock:) withObject:[[block copy] autorelease] waitUntilDone:YES];
}
